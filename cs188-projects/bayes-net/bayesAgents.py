...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        leftProb = factor.getProbability(left)

        right = evidence.copy()
        right.update({FOOD_HOUSE_VAR: TOP_RIGHT_VAL, GHOST_HOUSE_VAR: TOP_LEFT_VAL})
        rightProb = factor.getProbability(right)

        leftExpectedValue = leftProb * WON_GAME_REWARD + \
                                (1 - leftProb) * GHOST_COLLISION_REWARD
        rightExpectedValue = rightProb * WON_GAME_REWARD + \
                                (1 - rightProb) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, e in probsAndOutcomes:
            expectedValue += prob * max(self.computeEnterValues(e, enterEliminationOrder))
