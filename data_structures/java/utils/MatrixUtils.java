...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int r = 1; r < am.length; r++) {
            for (int c = 0; c < am[0].length; c++) {
                double p1 = get(am, r - 1, c - 1);
                double p2 = get(am, r - 1, c);
                double p3 = get(am, r - 1, c + 1);
                am[r][c] += Math.min(Math.min(p1, p2), p3);
            }
        }
        return am;
    }

    /** Returns the M[R][C] if r and c are valid.
     *  Double.POSITIVE_INFINITY is returned otherwise */
    public static double get(double[][] m, int r, int c) {
        try {
            return m[r][c];
    ...
    /** Returns a transpose of the matrix M given. */
    ...
    public static int[] findVerticalSeam(double[][] m) {
        ...
        for (int r = m.length - 1; r >= 0; r--) {
            indices[r] = indexOfLowest(m[r], low, high);
            low = Math.max(indices[r] - 1, 0);
            high = Math.min(indices[r] + 1, m[0].length - 1);
        }
        return indices;
    }

    /** Takes an array, a LOW index, and a HIGH index, and returns the
     *  index of the smallest thing in that array M between those
     *  indices (inclusive). */
    public static int indexOfLowest(double[] m, int low, int high) {
        int minIndex = low;
    ...
    public static int[] findSeam(double[][] m, Orientation orientation) {
        double[][] energy = accumulate(m, orientation);
        if (orientation == Orientation.VERTICAL) {
            return findVerticalSeam(energy);
        } else {
            return findVerticalSeam(transpose(energy));
        }
    }
