...
class Player(object):
    ...
    def go_to(self, location):
        ...
        else: self.place = destination_place
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            if type(item) == Key:
                key = item
        if key:
            if self.place.get_neighbor(place).locked: 
                self.place.get_neighbor(place).locked = False
                print(place,"is now unlocked!")
            else: print(place,"is already unlocked!")
        else:
    ...
    def knapsack(self, max_weight, list_of_treasures):
        ...
        27
        """
        takable_treasures = [item for item in list_of_treasures if item.weight<=max_weight]
        if not len(takable_treasures): return 0
        else:
            first_item = takable_treasures[0]
            if len(takable_treasures) == 1: return first_item.value
            else:
                rest_list = takable_treasures[1::]
                max_value_with_first = first_item.value + \
                                        self.knapsack(max_weight-first_item.weight,rest_list)
                max_value_without_first = self.knapsack(max_weight,rest_list)
                return max(max_value_with_first,max_value_without_first)
