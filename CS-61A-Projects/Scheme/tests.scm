...
(define (sum-tenth x y) (+ (tenth x) (tenth y)))
...
(sum-tenth (sum-tenth (+ 1 2) 10) 22)
; expect 2.33
 
(if (= (sum-tenth 200 2) 20.2)
    'right 'wrong)
; expect right
...
(let ((x 1))
      (let ((y 2) (x y)) y))
; expect Error: unknown identifier: y

(define a 100)
(define f (mu (x y) (+ x y a)))
(f 3 4)
; expect 107

(define g (lambda (x a) (f (+ x x) a)))
(g 3 4)
; expect 14
...
(define (pow b n)
  (if (= n 0) 1
      (let  ( (floorhalve (lambda (x) (floor (/ x 2)))) )
          (let  ( (x (pow b (floorhalve n))) )
                (if (even? n)
                    (* x x)
                    (* b x x))))))
(pow 2 5)
...
(define (nodots c)
  (cond ((null? c) c)
        ((not (pair? c)) (list c))
        ((not (pair? (car c))) (append (nodots (car c)) (nodots (cdr c))))
        (else(cons (nodots (car c)) (nodots (cdr c))))))
...
; expect (3 5)
(intersect odds (list 2 4 6 8))  ; Empty list is ()
; expect ()         
...
(define (union s t)
    (cond ((empty? s) t)
          ((empty? t) s)((= (car s) (car t)) (cons (car s) (union (cdr s) (cdr t))))
          ((< (car s) (car t)) (cons (car s) (union (cdr s) t)))
          ((> (car s) (car t)) (cons (car t) (union s (cdr t))))
          (else (cons s t)) ; replace this line
          ))
...
(define tr (tree 20 (tree 19 (tree 10 (tree 4 (leaf 1) (leaf 8)) (tree 15 () (leaf 16)))())
            (tree 30 (leaf 25)(tree 35 (tree 33 () (leaf 34)) ()))))
...
(in? tr 20)  ; True or False
; expect True
(in? tr 26) ; True or False
; expect False
(in? tr 8)  ; True or False
; expect True
(in? tr 0)  ; True or False
; expect False         
(in? tr 36)  ; True or False
; expect False
(in? tr 35)  ; True or False
; expect True

(define (as-list t)
    (cond ((empty? t) nil)
          (else (append (as-list (left t)) 
                        (cons (label t) 
                              (as-list (right t)))))))
(as-list (leaf 3))
; expect (3)
(as-list ())
; expect ()
(as-list (tree 20 (leaf 19) (leaf 30)))
; expect (19 20 30)
(as-list (tree 20 (tree 19 (leaf 10) ()) (tree 30 (leaf 25) (leaf 35))))
; expect (10 19 20 25 30 35)
    ...
    (if (<= n 0)
      ...
      (let ((rest (split-at (cdr lst) (- n 1))))
        (cons (cons (car lst) 
              (car rest)) 
            (cdr rest))))))
(car (split-at '(1 2 3 4 5) 3))
; expect (1 2 3)
(cdr (split-at '(1 2 3 4 5) 3))
; expect (4 5)
(car (split-at '(1 2 3 4 5) 10))
; expect (1 2 3 4 5)    
(cdr (split-at '(1 2 3 4 5) 10))
; expect ()
(car (split-at '(0 1 1 2 3) 0))
; expect ()
(cdr (split-at '(0 1 1 2 3) 0))
; expect (0 1 1 2 3)   
