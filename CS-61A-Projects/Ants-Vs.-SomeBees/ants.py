...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if exit: self.exit.entrance = self
        # END Problem 2
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect,QueenAnt) and not insect.is_impostor: return
...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 4
    min_range,max_range = 0,None #can only throw as close as the one in its place
    ...
    def nearest_bee(self, hive):
        ...
        while entrance_count < self.min_range: 
        #get to the starting place where we start to look for bees
            place = place.entrance
            entrance_count += 1
            if place is None: return None #If place is none, return none

        in_range = True
        while in_range and len(place.bees) == 0: #keep looking if in range and no bees found
            place = place.entrance
            entrance_count += 1
            if self.max_range: #If we have a max_range, check if we are still in the range
                in_range = (entrance_count <= self.max_range)
            if place is None: return None #If place is none, return none

        if place is hive or not in_range: return None #return None if in hive or out of range
        else: return random_or_none(place.bees)
    ...
    def add_insect(self, insect):
        ...
        if not insect.watersafe:
            if isinstance(insect,Ant):
                ...
                self.bees[-1].reduce_armor(insect.armor)
        # END Problem 3A
...
# The WallAnt class
class WallAnt(Ant):
    """ A WallAnt has a large armor value and does nothing in each turn"""
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        else: self.eat_bee(random_or_none(self.place.bees))
        # END Problem 6B
...
class QueenAnt(ScubaThrower):  
    ...
    def action(self, colony):
        ...
        if self.is_impostor:
            ...
            while place.exit is not None:
                place = place.exit
                if (place.ant is not None) and not (place.ant in self.boosted_ants):
                    place.ant.damage *= 2
                    self.boosted_ants += [place.ant]
                if isinstance(place.ant,BodyguardAnt) \
                    and place.ant.ant is not None\
                    and not (place.ant.ant in self.boosted_ants):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        self.armor -= amount
        if self.is_impostor: self.place.remove_insect(self)
        elif self.armor<=0: bees_win()
        # END Problem 9
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    return lambda colony, *args: action(colony, *args) if colony.time%2==0 else None
    # END Problem EC
...
def make_stun(action):
    ...
    """
    # BEGIN Problem EC
    return lambda colony, *args: None  
    # END Problem EC
